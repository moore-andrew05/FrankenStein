...
public class Straightener_edit implements PlugIn {
	static boolean processStack;
	static boolean secretSauce;

 	public void run(String arg) {
		ImagePlus imp = IJ.getImage();
		//ImagePlus imp = new ImagePlus("/Users/amoore/projects/FrankenStein/test_images/image3_FUSED.tif_MAX.tif");
		Roi roi = imp.getRoi();
		imp.show();
		...
		if (width<=1 || isMacro || stackSize>1) {
			...
			gd.addCheckbox("Andrew's Special Recipe?", secretSauce);
			gd.showDialog();
	...
	public ImageProcessor straightenNORGB(ImagePlus imp, Roi roi, int width) {
		ImageProcessor ip2;
		if (imp.getBitDepth()==24 && roi.getType()!=Roi.LINE)
			ip2 = straightenRGB(imp, width);
		else if (imp.isComposite() && ((CompositeImage)imp).getMode()==IJ.COMPOSITE) {
			if (roi.getType()==Roi.LINE)
				...
				ip2 = straightenCompositeNORGB(imp, width);
		} else if (roi.getType()==Roi.LINE)
			ip2 = rotateLine(imp, width);
		else
			ip2 = straightenLine(imp, width);
	...
	ImageProcessor straightenCompositeNORGB(ImagePlus imp, int width) {
